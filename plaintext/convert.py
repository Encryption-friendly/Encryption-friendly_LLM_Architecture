...
def main():
    ...
    ################################################################
    weight_path = f"pre-trained_weights/{args.name}/model.safetensors"
    ...
    save_path = "processed_data/processed_data.pth"
    # save_path = "Set output path that will be used as the same name in HE model"
    
    #########################################################################
    #### task have to be among {cola, mnli, qnli, qqp, rte, sst2, stsb} #####
    #########################################################################
    task = args.task
    ...
    # SST-2: 67349
    # QNLI: 104743
    train_dict = {'rte': 2490, 'mrpc': 3668, 'cola': 8551, 'stsb': 5749, 'sst2': 67349, 'qnli': 104743}
    train_dataset_len = train_dict[task]
    print(f'The length of train dataset: {train_dataset_len}')
    ####################################################################################
    ...
    # for i in range(0, train_dataset_len):
    for i in range(0, 1):
        inp = torch.load(f'./fine-tuning_data/{task}/train_inputs/input{i}_2ly_{task}.pth')
        container.add(f'input_{i}', inp)
        inp = torch.load(f'./fine-tuning_data/{task}/train_masks/mask{i}_2ly_{task}.pth')
    ...
    # SST2: 872
    # QNLI: 5463
    eval_dict = {'rte': 277, 'mrpc': 408, 'cola': 1043, 'stsb': 1500, 'sst2': 872, 'qnli': 5463}
    eval_dataset_len = eval_dict[task]
    print(f'The length of eval dataset: {eval_dataset_len}')
    #################################################################################
    # Torch path should be set as the same path in grad_test.py file,              ##
    ...
    #MRPC
    # for i in range(0, eval_dict):
    for i in range(0, 1):
        inp = torch.load(f'./fine-tuning_data/{task}/eval_inputs/input{i}_2ly_{task}.pth')
        container.add(f'input_{i}', inp)
        inp = torch.load(f'./fine-tuning_data/{task}/eval_masks/mask{i}_2ly_{task}.pth')
        container.add(f'mask_{i}', inp)
